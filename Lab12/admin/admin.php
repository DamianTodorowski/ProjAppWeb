...
function PokazProdukty() {
    ...
    // Poprawiona nazwa tabeli na `categories`
    $query = "SELECT p.*, c.nazwa AS nazwa_kategorii FROM produkty p 
              LEFT JOIN categories c ON p.id_kategoria = c.id";
    $result = mysqli_query($link, $query);
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<td>' . htmlspecialchars($row['nazwa_kategorii'] ?: 'Brak kategorii') . '</td>';
    ...
    // Poprawione zapytanie do tabeli `categories`
    $kat_query = "SELECT id, nazwa FROM categories";
    $kat_result = mysqli_query($link, $kat_query);
    if (!$kat_result) {
        die("Błąd w zapytaniu SQL: " . mysqli_error($link));
    }
    while ($kat_row = mysqli_fetch_assoc($kat_result)) {
        echo '<option value="' . $kat_row['id'] . '">' . htmlspecialchars($kat_row['nazwa']) . '</option>';
    }
...
function DodajProdukt() {
    ...
    $id_kategoria = intval($_POST['id_kategoria']);
    ...
    $query = "INSERT INTO produkty (tytul, opis, cena_netto, podatek_vat, ilosc_magazyn, id_kategoria, data_wygasniecia, zdjecie)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($link, $query);
    mysqli_stmt_bind_param($stmt, 'ssddiibs', $tytul, $opis, $cena_netto, $vat, $ilosc_magazyn, $id_kategoria, $data_wygasniecia, $zdjecie);
...
function UsunProdukt() {
    ...
    $id = intval($_POST['id']); // Pobranie ID produktu z formularza
...
function EdytujProduktForm() {
    ...
    if ($produkt) {
        ...
        echo '<form method="post" enctype="multipart/form-data">
                ...
                Tytuł: <input type="text" name="tytul" value="' . htmlspecialchars($produkt['tytul']) . '" required><br>
                Opis: <textarea name="opis" required>' . htmlspecialchars($produkt['opis']) . '</textarea><br>
                Cena netto: <input type="number" step="0.01" name="cena_netto" value="' . $produkt['cena_netto'] . '" required><br>
                Ilość w magazynie: <input type="number" name="ilosc_magazyn" value="' . $produkt['ilosc_magazyn'] . '" required><br>
                VAT: <input type="number" step="0.01" name="vat" value="' . $produkt['podatek_vat'] . '" required><br>
                Kategoria: 
                <select name="id_kategoria" required>';
        ...
        // Generowanie opcji dla pola wyboru kategorii
        while ($kat_row = mysqli_fetch_assoc($kat_result)) {
            $selected = ($produkt['id_kategoria'] == $kat_row['id']) ? 'selected' : '';
            echo '<option value="' . $kat_row['id'] . '" ' . $selected . '>' . htmlspecialchars($kat_row['nazwa']) . '</option>';
        }

        echo '</select><br>
                Data wygaśnięcia: <input type="date" name="data_wygasniecia" value="' . $produkt['data_wygasniecia'] . '"><br>
                Zdjęcie (pozostaw puste, aby nie zmieniać): <input type="file" name="zdjecie" accept="image/*"><br>
...
function EdytujProdukt() {
    ...
    // Sprawdzamy, czy przesłano nowe zdjęcie
    $zdjecie = null;
    $zmiana_zdjecia = false;
    ...
    if ($zmiana_zdjecia) {
        $query = "UPDATE produkty SET tytul = ?, opis = ?, cena_netto = ?, podatek_vat = ?, ilosc_magazyn = ?, id_kategoria = ?, data_wygasniecia = ?, zdjecie = ? WHERE id = ?";
        $stmt = mysqli_prepare($link, $query);
        mysqli_stmt_bind_param($stmt, 'ssddiibsi', $tytul, $opis, $cena_netto, $vat, $ilosc_magazyn, $id_kategoria, $data_wygasniecia, $zdjecie, $id);
    } else {
        $query = "UPDATE produkty SET tytul = ?, opis = ?, cena_netto = ?, podatek_vat = ?, ilosc_magazyn = ?, id_kategoria = ?, data_wygasniecia = ? WHERE id = ?";
        $stmt = mysqli_prepare($link, $query);
        mysqli_stmt_bind_param($stmt, 'ssddiisi', $tytul, $opis, $cena_netto, $vat, $ilosc_magazyn, $id_kategoria, $data_wygasniecia, $id);
    }
